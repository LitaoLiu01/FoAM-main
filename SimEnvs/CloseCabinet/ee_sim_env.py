...
class TransferTask(BimanualViperXEETask):
    ...
    def initialize_episode(self, physics):
        ...
        elif 'middle' in config['task_name']:
            drawer_joints = np.array([0, 0.15, 0])
        elif 'bottom' in config['task_name']:
            drawer_joints = np.array([0.12, 0, 0])
        cabinet_state = np.concatenate((cabinet_pose, drawer_joints))
        cabinet_body_id = physics.model.name2id('cabinet_joint', 'joint')
        print('cabinet_state', cabinet_state)
        physics.data.qpos[cabinet_body_id:cabinet_body_id + 10] = cabinet_state
        super().initialize_episode(physics)
