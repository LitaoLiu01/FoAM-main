...
def get_args_parser():
    ...
    parser.add_argument('--huber_weight', action='store', type=float, help='Huber_loss Weight', default=1)
...
def build_DREAM_wo_MPH_model_and_optimizer(args_override):
    ...
    model = build_DREAM_wo_MPH_model(args)
    model.cuda()
...
def build_MT_ACT_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
...
def build_G_img_ACT_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
    ...
    model = build_G_img_ACT_model(args)
    model.cuda()
...
def build_baku_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
    ...
    #     },
    # ]
    # module = [p for p in model.parameters() if p.requires_grad]
    # print(module)
...
def build_rt1_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
