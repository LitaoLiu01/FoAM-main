...
from .mlp import MLP
from detr.models.agent.utils import TruncatedNormal
######################################### Deterministic Head #########################################
...
class DeterministicHead(nn.Module):
    ...
    def loss_fn(self, is_pad, dist, target, reduction="mean", **kwargs):
        # Use L1 loss
        a_hat = dist.loc
        a_hat = einops.rearrange(a_hat, "b t1 (t2 d) -> b t1 t2 d", t2=10, d=14)
        a_hat = a_hat.squeeze(1)
        target = einops.rearrange(target, "b t1 (t2 d) -> b t1 t2 d", t2=10, d=14)
        target = target.squeeze(1)
        l1 = F.l1_loss(target, a_hat, reduction='none')
        l1 = (l1 * ~is_pad.unsqueeze(-1)).mean()

        if reduction == "mean":
            loss = l1 * 20
        elif reduction == "none":
            loss = l1 * self.loss_coef
        elif reduction == "sum":
            loss = l1.sum() * self.loss_coef
        else:
